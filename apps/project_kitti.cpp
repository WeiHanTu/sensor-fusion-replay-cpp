...
void printHelp(std::ostream& output) {
  output << "Usage: project_kitti --dataset-root <daily-root> --drive <*_sync> "
...
[[nodiscard]] ProjectOptions parseArguments(int argument_count, char** argument_values) {
...
[[nodiscard]] int run(const ProjectOptions& options) {
  ...
  sfr::viz::OverlayResult overlay =
      sfr::viz::renderDepthOverlay(image, projected.visible_points,
  ...
  const sfr::viz::ProjectionArtifactResult artifacts = sfr::viz::writeProjectionArtifacts({
      ...
      .config = {.z_min_m = options.z_min_m,
                 ...
                 .depth_max_m = options.depth_max_m,
                 .point_radius_px = options.point_radius_px},
      .projection_counts = projected.counts,
  ...
  std::cout << "completed projection run: " << artifacts.run_directory.string() << '\n';
  return kExitSuccess;
}
